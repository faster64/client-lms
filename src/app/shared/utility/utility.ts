...
export class Utility {
  ...
  public static imageExtensionsString = Utility.imageExtensions.map(x => `.${x}`).join(',');
  public static videoExtensionsString = Utility.videoExtensions.map(x => `.${x}`).join(',');
  ...
  public static randomText() {
    return ((Math.random() + 1) * Math.random()).toString(36).substring(2);
  }
